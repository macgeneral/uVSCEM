...
def _import_extension_manager(monkeypatch: pytest.MonkeyPatch):
    if "json5" not in sys.modules:
        ...
        json5_stub.loads = lambda _value: {}  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
        monkeypatch.setitem(sys.modules, "json5", json5_stub)

    if "requests" not in sys.modules:
        requests_stub = types.ModuleType("requests")
        requests_stub.Response = object  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
    ...
    if "dependency_algorithm" not in sys.modules:
        ...
        dependency_stub.Dependencies = _Dependencies  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
    ...
    if "uvscem.api_client" not in sys.modules:
        ...
        api_stub.CodeAPIManager = _CodeAPIManager  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
    ...
    if "uvscem.code_manager" not in sys.modules:
        ...
        code_stub.CodeManager = _CodeManager  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
